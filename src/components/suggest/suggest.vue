...
<template>
  ...
  >
    <ul class="suggest-list">
      ...
      <h5 class="noMore" v-else>没有更多数据了......</h5>
...
<script>
  ...
  import Laoding from 'base/loading/loading';
  ...
  export default {
    ...
    methods: {
      ...
      // 当向上滚动列表时派发一个事件(可让父组件对其进行操作，如向上滚动时失去焦点，手机键盘消失)
      listScroll() {
     ...
     /* filterSinger(singer) {
      ...
      search() {
        // 当query改变时要将page设为1，因为是要重新开始搜索（每次上拉到底部时触发searchMore方法都会将page加1）
        this.page = 1
        // 第一次请求数据一般都是有更多数据需要加载的
        this.hasMore = true
        // 当query改变时将suggest滚动到顶部
